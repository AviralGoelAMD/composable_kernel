...
BIN=./bin/tile_example_flatmm_basic
PREC=fp8
VERBOSITY=2
...
for args in "${ARGS_LIST[@]}"; do
  ...
  # Format: "CodegenFlatmmPipeline: pipeline_AGmemBGmemCRegV1_16x64x256x256_16x16x64_16x16x4_0x0x0"
  PIPELINE=$(echo "$OUTPUT" | grep -E "(pipeline_.*|gemm_.*_pipeline_.*)" | head -1)
  ...
  # Format: "Run Flatmm kernel with DataType = fp8 M =1 N =2048 K =5120 ... : 0.00828542 ms, 2.53114 TFlops, 1266.68 GB/s,"
  PERF_LINE=$(echo "$OUTPUT" | grep "TFlops")

  # Extract verification result
  # Format: "The GPU verification result is: correct"
  # Format: "The CPU veification result is:correct"
  VERIFICATION=$(echo "$OUTPUT" | grep -E "(GPU|CPU).*verification result is:" | sed -n 's/.*verification result is: *\(.*\)/\1/p')
