...
__device__ AFragT load_A_row_major(AType const* input_ptr)
{
    ...
    using ARawT         = typename scalar_type<AFragT>::type;
    using AScalarChunkT = typename vector_type<ARawT, scalar_type<AFragT>::vector_size / num_chunks>::type;
...
__device__ BFragT load_B_col_major(BType const* input_ptr)
{
    ...
    using BScalarChunkT = typename vector_type<BRawT, scalar_type<BFragT>::vector_size / num_chunks>::type;
...
__global__ void matmul(const packed_type_t<AType>* a, const packed_type_t<BType>* b, CType* c)
{
    ...
    using AFragT = typename vector_type<PackedAType, BLOCK_M * BLOCK_K / WAVE_SIZE / packed_size_a>::type;
    using BFragT = typename vector_type<PackedBType, BLOCK_K * BLOCK_N / WAVE_SIZE / packed_size_b>::type;
