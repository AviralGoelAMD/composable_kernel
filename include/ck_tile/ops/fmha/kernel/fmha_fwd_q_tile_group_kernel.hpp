...
struct FmhaFwdQTileGroupKernel
{
                                                ...
                                                bool has_padded_seqlen_k = false)
    {
        ...
        const index_t num_total_tile_0 = batch_size_ * nhead_ * num_tile_per_hdim_v;

        index_t num_workgroup_per_seqlen_q = 1;
        bool found                         = false;

        const index_t num_tile_per_seqlen_q =
            ck_tile::integer_divide_ceil(seqlen_q_, FmhaPipeline::kM0);

        while(num_workgroup_per_seqlen_q < num_tile_per_seqlen_q / 5)
        {
            if((num_total_tile_0 * num_workgroup_per_seqlen_q) % (num_cu * kBlockPerCu) == 0)
            {
                // found a solution where the total work-groups will be completely divided by
                // num_cu * kBlockPerCu
                found = true;
                break;
            }
            num_workgroup_per_seqlen_q++;
        };

        if(!found)
        {

            const index_t num_total_tile = num_total_tile_0 * num_tile_per_seqlen_q;

            // maximun number of tiles in each q_tile group
            constexpr index_t max_q_tile_group = 10;

            num_workgroup_per_seqlen_q = 1;

            // first ensure the number of work-groups is enough to cover the CUs
            while(num_total_tile_0 * num_workgroup_per_seqlen_q < num_cu * kBlockPerCu)
                num_workgroup_per_seqlen_q++;

            // next, further increase the number of work-groups until each group has not more than
            // 10 tiles
            while(ck_tile::integer_divide_ceil(num_total_tile,
                                               num_total_tile_0 * num_workgroup_per_seqlen_q) >
                  max_q_tile_group)
            {
                num_workgroup_per_seqlen_q++;
            };
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
        ...
        // divide problem
        const auto [i_tile_group_m, i_tile_n, i_nhead, i_batch] = GetTileIndex(kargs);

        const index_t i_m0_step =
            __builtin_amdgcn_readfirstlane(num_workgroup_per_seqlen_q * FmhaPipeline::kM0);
        index_t i_m0 = __builtin_amdgcn_readfirstlane((last_tile_per_seqlen_q - i_tile_group_m) *
                                                      FmhaPipeline::kM0);
